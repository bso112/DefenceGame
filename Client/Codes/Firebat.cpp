...
CFirebat::CFirebat(PDIRECT3DDEVICE9 pGraphic_Device)
	:CUnit(pGraphic_Device)
{
}

CFirebat::CFirebat(const CFirebat & _rhs)
	: CUnit(_rhs)
{
}

HRESULT CFirebat::Ready_GameObject_Prototype()
{
	return S_OK;
}

HRESULT CFirebat::Ready_GameObject(void * pArg)
{
	if (nullptr != pArg)
...
_int CFirebat::Update_GameObject(_double TimeDelta)
{
...
_int CFirebat::Late_Update_GameObject(_double TimeDelta)
{
...
HRESULT CFirebat::Render_GameObject()
{
...
HRESULT CFirebat::OnKeyDown(_int KeyCode)
{

	return S_OK;
}

void CFirebat::OnCollisionEnter(CGameObject * _pOther)
{
	CUnit* pUnit = dynamic_cast<CUnit*>(_pOther);
	if (nullptr != pUnit)
	{
		CTransform* pOtherTransform = (CTransform*)pUnit->Find_Component(L"Com_Transform");
		if (nullptr == pOtherTransform) return;
		...
		pOtherTransform->AddForce(_float3(-m_pBoxCollider->Get_CollDistance().x * 10, 0.f,0.f));
		else if(vOtherPos.x > vPos.x)
		pOtherTransform->AddForce(_float3(m_pBoxCollider->Get_CollDistance().x * 10, 0.f, 0.f));

		if (vOtherPos.z < vPos.z)
		pOtherTransform->AddForce(_float3(0.f, 0.f, -m_pBoxCollider->Get_CollDistance().z * 10));
...
void CFirebat::OnCollisionStay(CGameObject * _pOther)
{
	

}

CFirebat * CFirebat::Create(PDIRECT3DDEVICE9 pGraphic_Device)
{
	CFirebat*	pInstance = new CFirebat(pGraphic_Device);

	if (FAILED(pInstance->Ready_GameObject_Prototype()))
	{
		MSG_BOX("Failed To Create CFirebat");
...
CGameObject * CFirebat::Clone_GameObject(void * pArg)
{
	CFirebat*	pInstance = new CFirebat(*this);
