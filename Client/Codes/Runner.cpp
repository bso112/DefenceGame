...
CRunner::CRunner(PDIRECT3DDEVICE9 pGraphic_Device)
	:CUnit(pGraphic_Device)
{
}

CRunner::CRunner(const CRunner & _rhs)
	: CUnit(_rhs)
{
}

HRESULT CRunner::Ready_GameObject_Prototype()
{
	return S_OK;
}

HRESULT CRunner::Ready_GameObject(void * pArg)
{
	if (nullptr != pArg)
	...
	tTransformDesc.SpeedPerSec = 7.0;
	if (FAILED(Add_Component(SCENE_STATIC, L"Component_Transform", L"Com_Transform", (CComponent**)&m_pTransform, &tTransformDesc)))
		return E_FAIL;

	if (FAILED(Add_Component(SCENE_STATIC, L"Component_Texture_Cube", L"Com_Texture", (CComponent**)&m_pTexture)))
		return E_FAIL;
	...
	m_iRecogRange = 10;
	m_tUnitStats.iAtt = CValue<int>(10);
	m_tUnitStats.bFriendly = false;

	return S_OK;
}

_int CRunner::Update_GameObject(_double TimeDelta)
{
...
_int CRunner::Late_Update_GameObject(_double TimeDelta)
{
...
HRESULT CRunner::Render_GameObject()
{
	...
	if (FAILED(m_pTexture->Set_TextureOnShader(m_pShader, "g_DiffuseTexture", 3)))
		return E_FAIL;
...
HRESULT CRunner::OnKeyDown(_int KeyCode)
{

	return S_OK;
}

CRunner * CRunner::Create(PDIRECT3DDEVICE9 pGraphic_Device)
{
	CRunner*	pInstance = new CRunner(pGraphic_Device);

	if (FAILED(pInstance->Ready_GameObject_Prototype()))
	{
		MSG_BOX("Failed To Create CRunner");
...
CGameObject * CRunner::Clone_GameObject(void * pArg)
{
	CRunner*	pInstance = new CRunner(*this);
